...
def main():
    ...
    for track in tracks:
        if track:
            ...
            for idx, (measurement, state) in enumerate(track['measurements'], 1):
                # Extract measurement time from the measurement tuple
                measurement_time = measurement[3]  # Assuming time is the 4th element
                print(f"  Measurement {idx}: Time: {measurement_time}, State: {state}, Details: {measurement}")
    ...
    with open('updated_filter_states.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Track ID', 'Current State', 'Hit Count', 'Miss Count', 
                         'Measurement History', 'Track Status', 'Final Sf', 'Final Pf', 'Final Pp'])
        for track in tracks:
            if track:
