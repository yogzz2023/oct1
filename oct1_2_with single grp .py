...
def main():
        ...
        if len(group) > 1:  # Multiple measurements in the group
            ...
            if len(group) == 1: 
                ...
                for track_id, track in enumerate(tracks):
                    ...
                    # Handle 'free' state by setting it to 'unknown'
                    if current_state == 'free':
                        state_map[track_id] = 'unknown'
                        track['current_state'] = 'unknown'
                        print(f"Track {track_id} state set to 'unknown'.")

                    # Initialize filter state if in 'Poss1' or 'Tentative1'
                    if current_state == 'Poss1' or current_state == 'Tentative1':
                        print("Initializing filter state...")
                        kalman_filter.initialize_filter_state(x, y, z, 0, 0, 0, mt)
                        assigned = True
                        track['Sf'] = kalman_filter.Sf.copy()
                        track['Pf'] = kalman_filter.Pf.copy()
                    ...
                    # Perform prediction and update if in 'Firm'
                    elif current_state == 'Firm':
                        print("Performing predict and update step...")
                        kalman_filter.predict_step(mt)
                        Z = np.array([[x], [y], [z]])
                        kalman_filter.update_step(Z)
                        assigned = True
                        ...
                        # Update track's state if necessary
    ...
    for track in tracks:
        if track:
            ...
            # Measurement details for states
            measurements_detail = {}
            for measurement, state in track['measurements']:
                measurement_time = measurement[3]  # Assuming time is in the 4th position
                if state not in measurements_detail:
                    measurements_detail[state] = []
                measurements_detail[state].append((measurement_time, measurement))

            for state in ['Poss1', 'Tentative1', 'Firm']:
                if state in measurements_detail:
                    print(f"{state} Measurements:")
                    for m_time, measurement in measurements_detail[state]:
                        print(f"  - Time: {m_time}, Details: {measurement}")

            print("Measurement History:")
            for idx, (measurement, state) in enumerate(track['measurements'], 1):
                print(f"  Measurement {idx}: State: {state}, Details: {measurement}")
        ...
        with open('updated_filter_states.csv', 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Track ID', 'Current State', 'Hit Count', 'Miss Count', 'Measurement History', 
                            'Final Sf', 'Final Pf', 'Final Pp', 'Track Status', 'Measurement Details'])
            for track in tracks:
                if track:
                    measurements_str = '; '.join([f"({m[0]}, {m[1]}, {m[2]}, {m[3]}, {s})" for m, s in track['measurements']])
                    measurement_details = {state: [] for state in ['Poss1', 'Tentative1', 'Firm']}
                    for measurement, state in track['measurements']:
                        measurement_details[state].append(measurement)

                    measurement_details_str = {state: f"Count: {len(details)}, Times: {', '.join([str(m[3]) for m in details])}" 
                                                for state, details in measurement_details.items()}
                    writer.writerow([
                        track['track_id'],
                        track['current_state'],
